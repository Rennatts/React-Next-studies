...
function expensiveIncludes(haystack: string, needle: string) {
  // Artificial cost so the demo shows the difference.
  let score = 0;
  for (let i = 0; i < 1500; i += 1) score += i % 3;
  void score;
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
        ...
        <p className="mt-2 text-sm text-zinc-700 dark:text-zinc-300">
          Type to filter a large list. The input uses the{" "}
          <span className="font-medium">immediate</span> value; the expensive list uses the{" "}
          <span className="font-medium">deferred</span> value.
        </p>
      </div>
